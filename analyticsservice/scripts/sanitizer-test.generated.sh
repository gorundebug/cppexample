...
start_service() {
    ...
    container="${SANITIZER_CONTAINER_NAME:-cppexample-analyticsservice-$sanitizer}"
    docker network inspect "$network" >/dev/null 2>&1 || docker network create "$network" >/dev/null
    docker rm -f "$container" >/dev/null 2>&1 || true
    port_args=()
    port_args+=(-p "${SANITIZER_HOST_HTTP_PORT:-9093}:9093")
    port_args+=(-p "${SANITIZER_HOST_GRPC_PORT:-9203}:9203")

    docker run -d --name "$container" \
      --network "$network" \
      --network-alias "analyticsservice" \
      "${port_args[@]}" \
      -v "cppexample_analyticsservice_cpp-cmake-build:/workspace/build" \
      -w /workspace/source \
      -e SANITIZER_HOLD=1 \
