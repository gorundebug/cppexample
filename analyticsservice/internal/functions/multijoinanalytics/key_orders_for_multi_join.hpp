...
inline userver::engine::TaskWithResult<std::unique_ptr<KeyOrdersForMultiJoin>> MakeKeyOrdersForMultiJoin(
    servicelib::Context context, servicelib::IServiceEnvironment& environment) {
  return userver::utils::Async(
      "make-key_orders_for_multi_join", [context = std::move(context), &environment]() mutable {
        (void)context;
        (void)environment;

        return std::make_unique<KeyOrdersForMultiJoin>();
      });
}
