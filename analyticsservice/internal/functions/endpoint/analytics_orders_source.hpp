...
struct AnalyticsOrdersSource final
  ...
  void start(
    ...
    consumer(servicelib::MessageContext{},
             servicelib::Payload<Event>::make(Event{"high-value", 10, "order"}));
    consumer(servicelib::MessageContext{},
             servicelib::Payload<Event>::make(Event{"standard", 1, "order"}));
  }
...
inline userver::engine::TaskWithResult<std::unique_ptr<AnalyticsOrdersSource>> MakeAnalyticsOrdersSource(
  ...
  return userver::utils::Async(
      "make-analytics_orders_source", [context = std::move(context), &environment,
                             config]() mutable {
        (void)context;
        (void)config;
        (void)environment;
        return std::make_unique<AnalyticsOrdersSource>();
      });
}
