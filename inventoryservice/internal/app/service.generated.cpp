...
void ServiceGenerated::initMakers() {
  ...
  makers_.get_inventory_item_error = [](
      servicelib::Context context, servicelib::IServiceEnvironment& environment,
      const servicelib::config::MapStreamConfig& config) {
    return functions::MakeGetInventoryItemError(
        std::move(context), environment, config);
  };
...
void ServiceGenerated::initFunctions(
  ...
  maker_tasks.push_back(userver::utils::Async(
      "service-function-maker-get_inventory_item_error", [this, &cfg, maker_context,
                                            &maker_cancellation,
                                            &maker_error_mutex,
                                            &first_maker_error] {
        try {
          auto maker_task = makers_.get_inventory_item_error(
              maker_context, *this, cfg.streams.mapInventoryItemError);
          functions_.get_inventory_item_error = maker_task.Get();
          if (!functions_.get_inventory_item_error) {
...
void ServiceGenerated::initStreams(const config::Config& cfg) {
  SERVICEGEN_BUILD_INVENTORYITEM_STREAMS;
}
