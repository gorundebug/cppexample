...
if [[ -n "${SERVICEGEN_DEPENDENCY_PROXY_DIR:-}" ]]; then
  ...
  export SERVICELIB_SOURCE_CONTEXT="${SERVICELIB_SOURCE_CONTEXT:-${servicegen_proxy_base}/github-raw/gorundebug/cppservicelib/archive/refs/tags/v0.2.13.tar.gz}"
  export USERVER_SOURCE_CONTEXT="${USERVER_SOURCE_CONTEXT:-${servicegen_proxy_base}/github-raw/userver-framework/userver/archive/c9f77729c0edce7e423def2d4a4450aa7fc9d259.tar.gz}"
fi